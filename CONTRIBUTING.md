# Contributing to ThinkboxMYLibrary

Thanks for your interest in contributing to ThinkboxMYLibrary! ❤️
...
You will need Python 3.10 or later and the C++ compiler for your platform installed before you are able to build ThinkboxMYLibrary. The compilers used for each platform are as follows:
...
ThinkboxMYLibrary uses the C++17 standard.
...
ThinkboxMYLibrary also depends on the Maya SDK which is not available through any public Conan remotes. It You can follow these same steps in the MayaSDKConan subdirectory to generate a local Conan package containing it if you have Maya installed on your system.

## Building

From the project directory run the following commands to install the dependencies and build ThinkboxMYLibrary:
...
If you are using Windows, once run, you can open `build/ThinkboxMYLibrary.sln` in Visual Studio to use Visual Studio for development and debugging.

### Publishing to Local Conan Cache

If you need to publish build artifacts to your local Conan cache manually, after completing the [building](#building) steps, you can run the following commands to package ThinkboxMYLibrary and publish it to your local Conan cache:
